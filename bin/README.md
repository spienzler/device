...
1. **Login** — posts the configured username and password to `cmd=Login` and
   reads back an authentication token (using `jq`).
2. **Snapshot** — calls `cmd=Snap` with that token and writes the returned JPEG
   to the output path you provide.
...
> `rl.env` holds a plaintext password — keep it out of version control.
...
```bash
./bin/rl-snap.sh /path/to/snapshot.jpg
```

The image is written to the given path (via `wget -O`). If the file already
